...
const hourly = async (m, db) => {
    let user = db.users[m.sender]
    let __timers = (new Date - user.lasthourly)
    let _timers = (3600000 - __timers) // 1 jam
    let timers = clockString(_timers)

    if (new Date - user.lasthourly > 3600000) {
        m.reply(`*Hourly Claim*\n_Berhasil Claim_\n- limit : 5\n- uang : 1000\n\n_Claim reset tiap 1 jam_`)
        db.users[m.sender].limit += 5
        db.users[m.sender].money += 1000
        db.users[m.sender].lasthourly = new Date * 1
    } else {
        m.reply(`Silahkan tunggu *⏱️${timers}* lagi untuk bisa mengclaim hourly`)
    }
			}
...
const minutesly = async (m, db) => {
    let user = db.users[m.sender]
    let __timers = (new Date - user.lastminutesly)
    let _timers = (60000 - __timers) // 1 menit
    let timers = clockString(_timers)

    if (new Date - user.lastminutesly > 60000) {
        m.reply(`*Minutesly Claim*\n_Berhasil Claim_\n- limit : 1\n- uang : 5000\n\n_Claim reset setiap 1 menit_`)
        db.users[m.sender].limit += 1
        db.users[m.sender].money += 5000
        db.users[m.sender].lastminutesly = new Date * 1
    } else {
        m.reply(`Silahkan tunggu *⏱️${timers}* lagi untuk claim Minutesly`)
    }
}
